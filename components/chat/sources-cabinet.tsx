...
export function SourcesCabinet({
  ...
  return (
    <aside
      className={cn(
        "flex h-full min-h-0 w-full flex-col border-border/40 border-l bg-card/80 backdrop-blur-xl md:w-[32%] md:min-w-[280px]",
        className
      )}
    >
      <div className="flex items-center justify-between gap-2 border-border/40 border-b px-4 py-3">
        <p className="font-label text-muted-foreground uppercase">Sources</p>
        <Button
      ...
      <div className="flex gap-1 border-border/40 border-b px-2 py-2">
        {TABS.map(({ id, label, icon: Icon }) => (
          <button
            className={cn(
              "inline-flex flex-1 items-center justify-center gap-1 rounded-md px-2 py-1.5 font-medium text-xs transition-colors",
              tab === id
          ...
          webSources.length === 0 ? (
            ...
            <ul className="space-y-3">
              {webSources.map((source) => (
                <li key={source.url}>
                  <a
                    className="block rounded-lg bg-surface-container-low/80 p-3 transition-colors hover:bg-surface-container"
                    href={source.url}
        ...
        {tab === "memory" ? (
          <div className="space-y-2">
            ...
            <textarea
              ...
              className="min-h-[200px] w-full resize-none rounded-lg bg-surface-container-low p-3 text-sm outline-none ring-1 ring-border/40 focus:ring-2 focus:ring-primary/40"
