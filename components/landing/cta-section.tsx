...
export function CtaSection() {
  return (
    <section className="relative border-border/40 border-t py-20 sm:py-28">
      ...
      <div
        aria-hidden="true"
        className="-top-32 -translate-x-1/2 pointer-events-none absolute left-1/2 size-[640px] rounded-full bg-[var(--landing-accent)]/5 bg-[var(--landing-accent-hover)]/5 blur-[140px]"
      />
      ...
      <div className="relative mx-auto flex max-w-[820px] flex-col items-center px-4 text-center sm:px-6 lg:px-12">
        ...
        <h2 className="mt-5 font-display text-3xl font-semibold tracking-tight text-foreground sm:text-5xl lg:text-6xl">
          ...
          <span className="text-[var(--landing-accent)]">AI-native.</span>
        </h2>
        ...
        <div className="mt-8 flex w-full flex-col gap-3 sm:w-auto sm:flex-row">
          <Button
            asChild
            className="group h-12 w-full gap-2 rounded-full bg-[var(--landing-accent)] bg-[var(--landing-accent-hover)] text-white dark:text-[#131314] hover:opacity-90 border-0 sm:w-auto font-medium"
            size="lg"
          >
            <Link href="/chat">
              Generate MCP
          ...
          {FOOTNOTES.map((note) => {
            ...
            return (
              <span className="flex items-center gap-1.5" key={note.label}>
                <Icon className="size-3.5 text-[var(--landing-accent)]" />
                {note.label}
              </span>
            );
          })}
